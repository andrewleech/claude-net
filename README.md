...
The dashboard sends back session ids and nothing else; the daemon re-derives each cwd from its own fresh scan, so no caller-supplied path reaches the launcher. Restore therefore has no path-traversal surface. Rescanning on every request also means a dashboard left open overnight can't resurrect a session that has since come back by other means, because any id missing from the fresh scan is rejected. Restore is rate-limited to 3 batches per 5 minutes per host, 20 sessions per batch.

The same trust-model note as `launch` applies, and more so: `/api/host/<id>/recoverable` hands back every crashed project's absolute path and a redacted 140-character tail of its last prompt, and a restore batch can bring up 20 Claude Codes at once. Both routes assume the hub is not publicly reachable.
