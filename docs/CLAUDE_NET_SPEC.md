...
Developers running multiple Claude Code sessions concurrently who want those sessions to coordinate — delegate tasks, share results, or collaborate in named teams.
...
- `to` — recipient name or `team:teamname`
...
All outbound frames include an optional `requestId` string. The hub echoes it in the response frame when present. Operations that need confirmation (register, send_message, join_team, etc.) should include a `requestId`. Fire-and-forget operations may omit it.
